...
final class InsertionOnlyItemHandler implements IItemHandler {
    ...
    InsertionOnlyItemHandler(InsertionHandler handler) {
    ...
    public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
        ...
        var insertedAmount = handler.handle(variant, maxAmount, simulate);
        if (insertedAmount <= 0) {
            return stack;
        }
        var notInserted = maxAmount - insertedAmount;
        return notInserted > 0 ? variant.toStack(notInserted) : ItemStack.EMPTY;
    }
